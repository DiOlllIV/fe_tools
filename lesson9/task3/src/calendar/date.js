import { generateNumbers, renderTimeColumn } from './events';
import { moveRedline, setRedline } from './redline';
import { renderEventItem } from './renderEvents';
import { clickOnItems } from './popup';
import './date.scss';
...
export const setCurrMonth = () => {
  ...
  for (const arg of withoutFirst) {
    if (arg.textContent === 1) { checkMonthInWeek = true; }
  }

  const currMonth = month[new Date(today).getMonth()];
